...
1. **Expand `usda_nutrition.json` to ≥1,500 entries.** 377 is well below the DESIGN target of 7,793. Each gap forces the matcher to fall back to GI=55 with CL=0, silently halving accuracy. Consider a Python script in `scripts/` that pulls from USDA FoodData Central's CSV export and applies a curated whitelist. Production log export (2026-04-28) confirmed silent zeroes on `pine nuts`, `shrimp`, `chana masala`, `corn on the cob`, `gooseberries`, `cantaloupe`. Also add specific GI entries for `whole wheat spaghetti` (T5 unrecognized in prod), `tomato sauce` (fuzzy-fell to `tomato juice`), `honey wheat bread`.
