...
**Date-scoped physics scenes must use `.id(SceneKey)` on a child host view that constructs its scene at init.** `PhysicsBucketView` and `BalanceScaleView` each accept a `dateKey: Date?` init param and each contain a small private host view (`BucketSceneHost`, `BalanceSceneHost`). The host owns the SKScene as `@State`, initialized synchronously from `entries` in its `init`. The parent applies `.id(SceneKey(replay, dayKey, entryIDs, width, height))` to the host, so SwiftUI tears down and re-inits the host whenever any reactive input changes — and the new init reads the latest `entries` value passed in.
